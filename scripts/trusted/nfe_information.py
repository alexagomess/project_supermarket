...
from scripts.common.config import FOLDER_CLEANED_NFE_INFORMATION
...
class TrustedNFEInformation(BaseETL):
    ...
    def load_postgres(self, df: pd.DataFrame):
        ...
        df = self.preprocess_dates(df, date_columns)
    ...
    def execute(self):
        try:
            df = self.read_google_drive(folder_id=self.folder)
